...
- **商品別集計**（商品名・合計数・合計金額）
- **Excel用CSV保存**（レシート履歴・商品別集計）
- **商品マスター管理**（画面上での商品登録、CSVエクスポート／インポート、デフォルト復元）
...
商品一覧の登録フォームから、コード・商品名・単価を入力して商品を追加できます。追加した商品はブラウザの localStorage に保存されます。

カートに登録した商品は、カート行の `×` ボタンから確認後に削除できます。テンキー操作では `-` + Enter で直前の商品を削除できます。

ヘッダーの **商品管理** ボタンから、CSVファイルのインポート／エクスポートおよびデフォルトへのリセットもできます。
...
`*` キーで、レシート履歴 → 商品別集計（商品名・合計数・合計金額） → 閉じる、の順に切り替えます。

レシート履歴と商品別集計は、それぞれ **Excel用に保存** からCSVとして保存できます。レシート履歴CSVには、先頭にレシート件数と合計売上の集計も含まれます。
